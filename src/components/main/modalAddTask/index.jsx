                ...
                <div className="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
                    <DialogPanel transition className="relative transform overflow-hidden rounded-lg bg-white text-left shadow-xl transition-all data-closed:translate-y-4 data-closed:opacity-0 data-enter:duration-300 data-enter:ease-out data-leave:duration-200 data-leave:ease-in sm:my-8 sm:w-full sm:max-w-lg data-closed:sm:translate-y-0 data-closed:sm:scale-95" >
                        <form onSubmit={onSubmit} className="TodoForm">
                            <div className={`${stateDarkMode ? 'bg-white text-gray-900' : 'dark:bg-gray-800 dark:text-white'} flex flex-col md:flex-row max-w-full w-screen`}>
                                <div className='block w-full md:w-1/6 md:mr-1  px-4 py-2'>
                                ...
                                <div className='block w-full md:w-5/6 md:ml-1'>
                                    <div className='flex flex-col max-w-full w-screen'>
                                        ...
                                        <div className='block px-4 py-2'>
                                            <textarea className='w-full my-2 py-1.5 pr-3 pl-1 border-2 placeholder:text-gray-500 placeholder:italic rounded-2xl' placeholder="Registre la nueva tarea..." required={true} value={dateInputNew} onChange={onNewValueChange}></textarea>
                                        </div>
                                    </div>
                                </div>
                            </div>
